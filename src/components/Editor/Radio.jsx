  ...
  return (
    <div className={`flex items-center gap-1 ${ getIndentClass(indent) }`}>
      <div className={`grid place-items-center ${isChecked ? 'opacity-100' : 'opacity-70'}`}>
        <input id={`rd-${id}`} type="radio" name={name} value={value} className="appearance-none col-start-1 row-start-1 w-4 h-4 bg-content/20 rounded-full border border-content-700 z-10" onChange={onChange} checked={isChecked} disabled={isDisabled} />
      ...
      <label htmlFor={`rd-${id}`} className={`ml-1 text-content ${isChecked ? 'opacity-100' : 'opacity-70'}`}>{label}</label>
    </div>
  )
}
